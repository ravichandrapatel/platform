...
| Path | Purpose |
|------|--------|
| **actions/** | Composite reusable actions (use from workflows with `uses: org/repo/devtools-landingzone/actions/NAME@ref`) |
| **images/** | Containerfiles and build context for images (Buildah/Podman) |
| **workflows/** | Workflow definitions (run from `.github/workflows/`; paths reference `devtools-landingzone/`) |
| **act/** | Local testing with [act](https://github.com/nektos/act): run workflows from repo root via `./devtools-landingzone/act/run.sh` |
...
**Path:** `devtools-landingzone/actions/owasp-dependency-check/`
...
```yaml
- uses: YOUR_ORG/IDP/devtools-landingzone/actions/owasp-dependency-check@v1.0.0   # or @main for latest
  with:
    project: my-app
    path: .
    format: HTML
...
| Action | Path | Purpose |
|--------|------|---------|
| **owasp-dependency-check** | `devtools-landingzone/actions/owasp-dependency-check/` | OWASP Dependency-Check in CI (Podman, UBI9 image). |
| **git-path-filter** | `devtools-landingzone/actions/git-path-filter/` | Detect file changes between refs; YAML pattern groups; JSON output for monorepo path filters. |
| **drift-auditor** | `devtools-landingzone/actions/drift-auditor/` | Terraform drift: parallel `terraform plan` (S3 backend), single "Infrastructure Drift Report" Issue per repo. |
| **prbot** | `devtools-landingzone/actions/prbot/` | Create or reuse a PR (same or another repo). Idempotent. |
| **janitor-bot** | `devtools-landingzone/actions/janitor-bot/` | Scan/cleanup stale branches, PRs, artifacts, packages (repo/org/topic). Config via env. |

Details: see `readme.md` in each action folder.

### Adding a new action

1. Create **`devtools-landingzone/actions/<name>/`** with **`action.yml`** (or `action.yaml`) and a **`readme.md`** that describes purpose, inputs, outputs, and usage.
...
|-------|------|------|---------|
| **owasp-dependency-check** | `devtools-landingzone/images/owasp-dependency-check/` | UBI9 | OWASP Dependency-Check CLI (nightly build; compliance pulls and scans only). |
| **gha-runner-scale-set-runner** | `devtools-landingzone/images/gha-runner-scale-set-runner/` | UBI9 | ARC self-hosted runner with rootless Podman/Buildah. |
| **gha-runner-scale-set-controller** | `devtools-landingzone/images/gha-runner-scale-set-controller/` | UBI9 minimal | ARC controller (binaries copied from upstream; no distroless). |
<!-- IMAGES_TABLE_END -->

### Adding a new image

1. Create **`devtools-landingzone/images/<id>/`** with at least a **Containerfile** (and any scripts it needs).
2. Add optional **`devtools-landingzone/images/<id>/image-info.yaml`** with `name`, `base`, `purpose` for the README table.
3. Add an entry to **`devtools-landingzone/images/images.yaml`** (`id`, `pull_only`, optional `build_args`). The compliance workflow and README table pick it up automatically.
...
Workflows that **run** on GitHub live in **`.github/workflows/`** at the repo root. The **canonical source** for many of them is under **`devtools-landingzone/workflows/`**; the Platform Component Manager (promote mode) copies workflow files from `devtools-landingzone/workflows/<name>/` into `.github/workflows/` so they execute. Only files in `.github/workflows/` are run by GitHub.
...
| **Compliance** | `devtools-landingzone/workflows/compliance/` | Trivy per image; build+push when Critical=0; updates compliance table. |
| **Dependency-Check UBI9 (nightly)** | `devtools-landingzone/workflows/dependency-check-nightly/` | Build and push owasp-dependency-check image to GHCR. |
| **nodejs-app** | `devtools-landingzone/workflows/nodejs-app/` | Reusable Node.js app workflow. |
| **terraform** / **terraform-refresh-daily** | `devtools-landingzone/workflows/terraform/`, `terraform-refresh-daily/` | Terraform plan/apply; daily refresh. |
| **drift-check** | `devtools-landingzone/workflows/drift-check/` | Drift detection (uses drift-auditor action). |

Reusable workflows are called with `uses: org/repo/.github/workflows/name.yml@ref`.

**Reference vs released:** Workflows under `devtools-landingzone/workflows/compliance/` and `dependency-check-nightly/` are **published** to `.github/workflows/` (by repo setup or PCM). Others (e.g. `nodejs-app`, `drift-check`) are **reference** or released via PCM when you promote them.

### Adding a new reusable workflow

1. Create **`devtools-landingzone/workflows/<name>/`** with your workflow `.yml` and a **`readme.md`** that describes purpose, inputs, and when it runs.
...
- **Stable:** `uses: YOUR_ORG/IDP/devtools-landingzone/actions/owasp-dependency-check@1.0.0`
- **RC:** `uses: YOUR_ORG/IDP/devtools-landingzone/actions/owasp-dependency-check@1.0.0-rc`
...
1. Create or update the component under `devtools-landingzone/actions/<name>` or `devtools-landingzone/workflows/<name>`.
...
Local testing: `./devtools-landingzone/act/run.sh` (see [act/readme.md](act/readme.md)).
