# gha-runner-scale-set-controller (UBI9)
...
```bash
buildah build -t ghcr.io/YOUR_ORG/gha-runner-scale-set-controller:ubi9-0.13.1 -f Containerfile .
# or
podman build -t ghcr.io/YOUR_ORG/gha-runner-scale-set-controller:ubi9-0.13.1 -f Containerfile .
```

To use a different upstream tag:

```bash
podman build --build-arg CONTROLLER_TAG=0.13.0 -t ghcr.io/YOUR_ORG/gha-runner-scale-set-controller:ubi9-0.13.0 -f Containerfile .
```
